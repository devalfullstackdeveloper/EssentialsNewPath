...
class BOS1 extends Model
{
    use HasFactory;
 
    protected $table = "bos1_data";
	...
	public static function store($data,$system){
		...
		foreach ($data as $key => $value) {
			$checkExist = DB::table("bos1_data")->select('client_id')->where(["client_id"=>$value['client_id'],"import_flag"=>"1"])->get();
			if(count($checkExist)==0){
				...
				$updateBOS1 = \App\Helpers\UpdateHelper::Update_BOS1_intials($value['client_id'],$value);
