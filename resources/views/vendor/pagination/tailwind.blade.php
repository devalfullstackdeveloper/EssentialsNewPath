@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
      {{--   <div class="flex justify-between flex-1 sm:hidden">
        ...
        <div class="hidden myfooter sm:flex-1 sm:flex sm:items-center sm:justify-between">
            <div >
            ...
            <div class="" style="height: 100%;">
                <span class="relative z-0 inline-flex shadow-sm rounded-md myspan">
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                            ...
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-2 py-1 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-default leading-5 active:bg-gray-100 active:text-gray-500">{{ $page }}</span>
                                    </span>
                                @else
...
@endif
<style>
    span.relative.z-0.inline-flex.shadow-sm.rounded-md.myspan {
    position: relative;
    top: 20%;
}
</style>